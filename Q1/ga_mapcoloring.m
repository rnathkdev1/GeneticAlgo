function [solution,score] = ga_mapcoloring(varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while(Iter<5000)
    ...
    fprintf('Best Fitness for Iter: %d is %f\n',Iter,BestFitness);
    for i=1:size(population,1)
        fitness(i)=sum(population(i,row)~=population(i,col));
        
        if fitness(i)==fitness_MAX
            %I=fitness==max(fitness);
            [condition,solution]=checkCondition(population(i,:));
            fprintf('Generating Equality of States: Iter: %d\n',Iter)
            if condition==1
                BestFitness=max(fitness)*100/fitness_MAX;
